...
def get_player_id(name, team, position, client, table_name):
    ...
    try:
        for i in range(0,len(response.data)):
            if (response.data[i]['Player Name'] == name) and (response.data[i]['Team'] == team) and (response.data[i]['ESPN Roster Position'] == position):
                player_id = response.data[i]['id']
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    for index, row in schedule.fillna(0).iterrows():
        ...
        if response_check.data == []:
            ...
            data_to_insert['updated_at'] = dt.now()
            data_to_insert = to_jsonable(data_to_insert)
